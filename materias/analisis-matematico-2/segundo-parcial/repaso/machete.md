...
**El teórico reciclado** (cayó 6 veces, siempre igual): dan la transformación $(x,y)=T(u,v)$ — es decir, $T$ va **de $UV$ hacia $XY$** — y el área de $D$ (en el plano $XY$); piden el área de $D^*$ (en $UV$). Como $T$ manda $D^*$ en $D$:
...
| $T(u,v)$ | $J$ | Dato → resultado |
|---|---|---|
| $(v-2u,\;u+v)$ | $-3$ | área$(D)=9$ → área$(D^*)=3$ |
| $(u+3v,\;2u+2v)$ | $-4$ | área$(D)=6$ → área$(D^*)=3/2$ |
| $(u+2v,\;2u+v)$ | $-3$ | área$(D)=6$ → $2$ · área$(D)=12$ → $4$ |

- En el resultado va **siempre $\lvert J\rvert$**: un área jamás da negativa. (Fijate que los $J$ reales son todos negativos — el signo se descarta.)
- Variante V/F: dan $\iint_{D^*}(\dots)\,du\,dv$ y afirman cuánto vale $\iint_D x\,dx\,dy$. Herramienta: la misma igualdad de arriba con $\lvert J\rvert$, reemplazando $x=x(u,v)$; compará el integrando que te dan contra el que corresponde antes de contestar.
...
**Receta T2 "graficar y pasar a cartesianas":**
...
**Ejemplo completo:** volumen de $(x-1)^2+y^2\le z\le 5-2x$.
...
- Checksum: media elipse $\tfrac{x^2}4+y^2\le1$, $y\ge0$ → $A=\pi$, $\bar y=\dfrac{4}{3\pi}$, baricentro $\big(0,\tfrac{4}{3\pi}\big)$.
...
- **cuádrica ∩ cono/paraboloide** → escribí las dos en función de $\rho$ y $z$, sustituí una en la otra y resolvé la cuadrática. Ejemplo (reciclado): elipsoide $2\rho^2+z^2=3$ con cono $z=\rho$ → $2z^2+z^2=3$ → $z_0=\rho_0=1$: se cortan en la circunferencia $\rho=1$ a altura $z=1$ (y esa circunferencia define la proyección $\rho\le1$).
...
- Longitud: $L=\int_a^b\lVert\lambda'(t)\rVert\,dt$. Checksum: hélice $\lambda(t)=(2\cos t,2\operatorname{sen}t,2t)$ de $(2,0,0)$ a $(-2,0,2\pi)$: $\lVert\lambda'\rVert=\sqrt{4+4}=2\sqrt2$ cte → $L=2\sqrt2\,\pi$.
...
**Ejemplo de "intersección de 2 superficies":** trabajo de $f=(xy,\,y^2,\,xz)$ sobre $z=x+y$ ∩ $x=y^2$, de $(0,0,0)$ a $(4,2,6)$.
...
- El atajo que vale puntos: $f=(z,y,x)$ sobre $x^2+y^2=4$ ∩ $z=x$, de $(2,0,2)$ a $(-2,0,-2)$. Jacobiana simétrica ✓ → conservativo → $\varphi=xz+\tfrac{y^2}2$ → $W=\varphi(-2,0,-2)-\varphi(2,0,2)=4-4=\boxed{0}$. Cero integrales, cero parametrización.
- Si no es conservativo, igual puede simplificarse solo: con $f=(xy,-y^2,z^2)$ sobre $z=9-x^2$, $z=y$, de $(3,0,0)$ a $(0,9,9)$ (parámetro $x=t$), los términos de $Q$ y $R$ se cancelan entre sí y queda $\int_3^0(9t-t^3)\,dt=\boxed{-\tfrac{81}4}$.
...
- Curva paramétrica $\lambda(t)=(x(t),y(t))$: $A=\dfrac12\int_a^b\big(x\,y'-y\,x'\big)\,dt$ — así piden las áreas encerradas.
...
| Campo | $Q_x-P_y$ | Circulación antihoraria |
|---|---|---|
| $f=\big(\tfrac{xy^2}{2},\,\tfrac{3x^2y}{2}\big)$ | $2xy$ | $1/12$ |
| $f=(xy^2,\,3x^2y)$ | $4xy$ | $1/6$ |
| $f=\big(x^2+\varphi(y-x),\,x^2-\varphi(y-x)\big)$, $\varphi\in C^1$ desconocida | $2x$ (las $\varphi$ se cancelan) | $1/6$ |

**Función desconocida en el campo:** si $f$ trae una $g$ o $\varphi$ que no te dan y piden circulación cerrada → Green: las incógnitas **mueren en $Q_x-P_y$**. Ejemplo: $f=(y+g(x),\,2x+g(y))$ sobre la frontera de $x^2+y^2\le2y$ → $Q_x-P_y=2-1=1$ → circulación $=1\cdot\text{área}(D)=\boxed{\pi}$ (antihorario).
...
- Ojo fino: simplemente conexo es **suficiente, no necesario** — puede fallar y el potencial existir igual (caso real abajo).
...
**El caso con agujero en el dominio (T1 real), resuelto:** ¿$f=\Big(\dfrac{2x}{x^2+y^2},\,\dfrac{2y}{x^2+y^2}\Big)$ admite potencial?
...
**Variante "hallar $g(x)$"** (reciclada): plantear $P_y=Q_x$ (o $\operatorname{div}f=0$) → **suele quedar una EDO de 2° orden en $g$** → resolverla con la receta de §9; los datos tipo $f(0,1)=(0,7)$ fijan las constantes.

- Checksum (completo): $f=(x^2-4y\,g(x),\;g'(x)-x+y)$ → $P_y=Q_x$: $-4g=g''-1$ → $g''+4g=1$ → $g=C_1\cos 2x+C_2\operatorname{sen}2x+\tfrac14$. Datos $f(0,1)=(0,7)$: 1ª componente $-4g(0)=0$ → $g(0)=0$ → $C_1=-\tfrac14$; 2ª componente $g'(0)+1=7$ → $g'(0)=6$ → $C_2=3$. $\boxed{g=3\operatorname{sen}2x-\tfrac14\cos2x+\tfrac14}$

**T2 reciclado ×3:** $f=(2xy+2x\,g'(x^2),\;x^2)$, circulación de $(-2,4)$ a $(2,5)$.
...
- Checksums de área: cono $z=\sqrt{x^2+y^2}$ bajo $z=2$: $\sqrt2\cdot\text{área}(x^2+y^2\le4)=4\sqrt2\,\pi$. Y $z=x^2-y$ sobre $\lvert y\rvert\le x\le1$: $\sqrt{2+4x^2}$ no depende de $y$ → $\int_0^1 2x\sqrt{2+4x^2}\,dx=\sqrt6-\tfrac{\sqrt2}3$.

**El reciclado ×4:** flujo de $f=(y^2,\,z^2+x^2,\,x^2)$ a través del plano $y=x$ limitado por $x^2+y^2+2z^2\le2$.
...
**Flujo directo por cilindro corrido:** $f=(-y,\,x,\,xz)$ a través de $x^2+y^2=2x$ (abierta) con $z\le4-x^2-y^2$, 1er octante.
...
- $\operatorname{div}f=\text{cte}$ → $\Phi=\text{cte}\cdot\operatorname{Vol}(V)$ → usar los volúmenes conocidos de §2. Checksums: $\operatorname{div}=2$ en el tetraedro de interceptos $6,4,3$ ($V=12$) → $\Phi=24$; $\operatorname{div}=3$ con $V=\tfrac83$ → $\Phi=8$; $\operatorname{div}=6$ en medio tetraedro ($V=\tfrac23$) → $\Phi=4$.
- Si $\operatorname{div}$ no es constante, igual suele ser corta: $\operatorname{div}=2y$ sobre el tetraedro de interceptos $2,1,3$ → $\Phi=2\,V\bar y=2\cdot1\cdot\tfrac14=\tfrac12$ (el baricentro del tetraedro está en $\tfrac b4$).
- Componente desconocida que **no depende de su propia variable** → aporta $0$ a la div: $f=(g(y,z),\,h(x,z),\,z)$ → $\operatorname{div}f=1$; $f=(x-y-z,\,y-x-z,\,g(x,y))$ → $\operatorname{div}f=2$.
- "Hallar $g$ solenoidal": $\operatorname{div}f=0$ → queda una EDO en $g$ (puede ser de 2° orden → §9). Checksum: $f=(x+g'(x),\,y\,g(x),\,y^2-xz)$ → $1+g''+g-x=0$ → $g''+g=x-1$ → con $f(0,1,0)=(1,1,1)$: $g=2\cos x+x-1$.
- **"Flujo = volumen para todo radio":** $\iiint\operatorname{div}f\,dV=\operatorname{Vol}(V)$ para toda esfera ⟺ $\operatorname{div}f\equiv1$. Con $f=(x+g'(x),\,y\,g'(x),\,-2z\,g(x))$: $1+g''+g'-2g=1$ → $g''+g'-2g=0$ → con $f(0,0,1)=(2,0,0)$ ($g(0)=0$, $g'(0)=2$): $\boxed{g=\tfrac23\big(e^{x}-e^{-2x}\big)}$

**Superficie abierta vía Gauss:** cerrar con una tapa $T$ y despejar:
...
- Checksum: $f=(g(y,z),h(x,z),z)$, $S$: $z=1+x^2+y^2$ con $z\le2$, $\hat n$ hacia $z^+$. $\operatorname{div}=1$ → $\iiint=V=\tfrac\pi2$; tapa $z=2$ ($x^2+y^2\le1$, saliente $=+\hat k$): $\Phi_T=2\cdot\pi=2\pi$. En el paraboloide el saliente apunta hacia abajo (opuesto al pedido): $\Phi_{S}^{\,z^+}=\Phi_T^{\,z^+}-\iiint=2\pi-\tfrac\pi2=\boxed{\tfrac{3\pi}2}$
...
**Si te dan $\operatorname{rot}f$ y no $f$: Stokes obligado** (enunciado típico: la curva es la intersección de dos paraboloides):
...
- Checksum 1: $\operatorname{rot}f=(x,\,x^2-2x,\,-z)$, curva $z=3-x^2-y^2$ ∩ $z=2x^2+2y^2$ → $3-\rho^2=2\rho^2$ → $\rho_0=1$, $z_0=2$ → $\oint=\iint(-2)\,dA=\boxed{-2\pi}$ con $\hat n=+\hat k$ (la otra orientación: $+2\pi$).
- Checksum 2: $\operatorname{rot}f=(-x^2,\,3y-x,\,4z^2-3xy)$, curva $z=3(x^2+y^2)$ ∩ $z=4-(x^2+y^2)$ → $\rho_0=1$, $z_0=3$ → $\oint=\iint(36-3xy)\,dA=36\pi-0=\boxed{36\pi}$ ($\hat n=+\hat k$).

**El reciclado ×2 de circulación por Stokes:** $f=(yz,\,2xz,\,xy)$ sobre la intersección de $z=x^2+4y^2$ con $z=8-x^2-4y^2$ (o el espejo $4x^2+y^2$ y $8-4x^2-y^2$).
...
## 9 · EDO lineales de 2° orden (P4 típico, a veces P1 o T2)
...
- "Recta tangente $y=mx+c$ en $x_0$" → $y(x_0)=mx_0+c$ **y** $y'(x_0)=m$. Ya vino así dos veces — resueltas en §10.
...
|---|---|
| $y''+y'-2y=\cos x$ | $y=C_1e^{x}+C_2e^{-2x}+\frac{\operatorname{sen}x-3\cos x}{10}$ |
| $y''+4y'=8$ | $y=C_1+C_2e^{-4x}+2x$ |
| $y''+2y'=4x$ | $y=C_1+C_2e^{-2x}+x^2-x$ |
| $y''-6y'+9y=2x$ | $y=(C_1+C_2x)e^{3x}+\frac{2}{9}x+\frac{4}{27}$ |
| $y''-2y'+5y=2x$ | $y=e^{x}(C_1\cos 2x+C_2\operatorname{sen}2x)+\frac{2}{5}x+\frac{4}{25}$ |
| $y''-4y'+13y=26$ | $y=e^{2x}(C_1\cos 3x+C_2\operatorname{sen}3x)+2$ |
| $y''-3y'=2-6x$ | $y=C_1+C_2e^{3x}+x^2$ |
| $y''-2y'+5y=10$ | $y=e^{x}(C_1\cos 2x+C_2\operatorname{sen}2x)+2$ |
| $y''-y=4$ | $y=C_1e^{x}+C_2e^{-x}-4$ |
| $y''-y=x+1$ | $y=C_1e^{x}+C_2e^{-x}-x-1$ |
| $y''-6y'+9y=9x$ | $y=(C_1+C_2x)e^{3x}+x+\frac{2}{3}$ |

**Las dos particulares "con recta tangente en $(0,y_0)$", resueltas** (formato que ya se repitió — §9):

- $y''-6y'+9y=9x$ con tangente $y=x+2$ → $y(0)=2$, $y'(0)=1$ → $C_1=\tfrac43$, $C_2=-4$ → $y=\big(\tfrac43-4x\big)e^{3x}+x+\tfrac23$
- $y''-y=x+1$ con tangente $x+y=2$ → $y(0)=2$, $y'(0)=-1$ → $C_1=C_2=\tfrac32$ → $y=\tfrac32\big(e^{x}+e^{-x}\big)-x-1$

**Potenciales pedidos en parciales (el tema del 100%):**

| Campo y dato | Respuesta |
|---|---|
| $f=(2x+y^2+1,\,2xy)$, $\varphi(0,0)=2$ | $\varphi=x^2+xy^2+x+2$ |
| $f=(2x,\,2y)$, $\varphi(0,0)=1$ | $\varphi=x^2+y^2+1$ |
| $f=(2xy+1,\,x^2+2y)$, $\varphi(1,2)=5$ | $\varphi=x^2y+x+y^2-2$ |
| $f=(6xy+2y^2+2,\,3x^2+4xy-2)$, $\varphi(1,2)=11$ | $\varphi=3x^2y+2xy^2+2x-2y-1$; piden $\varphi(1,0)=1$ |
| $f=(4xy-2,\,2x^2-2y)$, $\varphi(1,1)=0$ | $\varphi=2x^2y-2x-y^2+1$ |
| $f=(12x+2yz,\,6y+2xz,\,2xy)$; ¿qué $a$ anula $\int_{(-a,a,1)}^{(1,a,a)}$? | $\varphi=6x^2+3y^2+2xyz$ → $6+5a^2=7a^2$ → $a=\pm\sqrt3$ |
| $f=\big(\tfrac{2x}{x^2+y^2},\,\tfrac{2y}{x^2+y^2}\big)$ | $\varphi=\ln(x^2+y^2)+C$ (§5) |

**Circulación / trabajo / Green / Stokes:**

| Problema | Resultado |
|---|---|
| Green ½: $f=(\tfrac{xy^2}{2},\tfrac{3x^2y}{2})$, frontera de $x^2\le y\le x$ (reciclado ×3) | $1/12$ (antihorario) |
| Green entero: $f=(xy^2,3x^2y)$, mismo recinto | $1/6$ |
| Green con $\varphi(y-x)$ desconocida, mismo recinto | $1/6$ |
| Green con $g$ desconocida: $f=(y+g(x),2x+g(y))$, frontera de $x^2+y^2\le2y$ | $\pi$ |
| Stokes con rotor dado $(x,\,x^2-2x,\,-z)$ (§8) | $-2\pi$ con $\hat n=+\hat k$ |
| Stokes con rotor dado $(-x^2,\,3y-x,\,4z^2-3xy)$ (§8) | $36\pi$ con $\hat n=+\hat k$ |
| Circulación $f=(yz,2xz,xy)$, intersección de paraboloides (reciclado ×2, §8) | $\pm8\pi$ según orientación |
| Trabajo $f=(z,y,x)$, curva $x^2+y^2=4$ ∩ $z=x$ (§3) | $0$ (conservativo) |
| Trabajo $f=(xy,y^2,xz)$ sobre $x=y^2$, $z=x+y$ (§3) | $60$ |
| Circulación $f=(xy,-y^2,z^2)$ de $(3,0,0)$ a $(0,9,9)$ (§3) | $-81/4$ |
| Potencial con $g$ cancelada, de $(-2,4)$ a $(2,5)$ (§5) | $4$ |

**Flujo / Gauss:**

| Problema | Resultado |
|---|---|
| Reciclado ×4: $f=(y^2,z^2+x^2,x^2)$ por $y=x$, $x^2+y^2+2z^2\le2$ (§6) | $\pm\pi/4$ según $\hat n$ |
| Gauss, $\operatorname{div}=6$, medio tetraedro ($V=2/3$) | $4$ (saliente) |
| Gauss, $\operatorname{div}=2y$, tetraedro de interceptos $2,1,3$ | $1/2$ (saliente) |
| Gauss, $\operatorname{div}=2$, tetraedro de interceptos $6,4,3$ | $24$ (saliente) |
| Gauss, $\operatorname{div}=3$, $V=8/3$ | $8$ (saliente) |
| Abierta con tapa, $\operatorname{div}=1$ (§7) | $3\pi/2$ hacia $z^+$ |
| Flujo abierto: $\operatorname{div}f=2z$, $z=4-x^2-y^2$, dato disco $7\pi$ hacia $z^+$ | $\frac{64\pi}{3}+7\pi=\frac{85\pi}{3}$ |
| Cilindro corrido $x^2+y^2=2x$ (§6) | $\pm4$ según $\hat n$ |
| Hallar $g$ solenoidal (§7) | $g=2\cos x+x-1$ |
| Flujo = volumen (§7) | $g=\tfrac23(e^{x}-e^{-2x})$ |
| Hallar $g$ conservativo (§5) | $g=3\operatorname{sen}2x-\tfrac14\cos2x+\tfrac14$ |

**Integrales múltiples / áreas / cambio de variables:**

| Problema | Resultado |
|---|---|
| Volumen $(x-1)^2+y^2\le z\le5-2x$ (§2) | $8\pi$ |
| Volumen elipsoide ∩ cono, mitad ($2\rho^2+z^2\le3$, $z\ge\rho$, medio espacio) | $\pi\,\frac{\sqrt3-1}{2}$ |
| Volumen $x^2+y^2\le2y$, $\lvert z\rvert\le2y$ | $4\pi$ |
| Masa cono ∩ esfera $\rho\le\sqrt{18}$, $\delta=kz$ | $\tfrac{81\pi k}{2}$ |
| Masa cilindro radio 2 entre planos de altura constante $5$, $\delta=k\rho$ | $\tfrac{80\pi k}{3}$ |
| Baricentro media elipse $\tfrac{x^2}4+y^2\le1$, $y\ge0$ | $\big(0,\tfrac{4}{3\pi}\big)$ |
| Área del cono $z=\sqrt{x^2+y^2}$ bajo $z=2$ | $4\sqrt2\,\pi$ |
| Área de $z=1-x^2-y^2$, $z\ge0$ (= apunte 12-Ej4) | $\frac{\pi}{6}(5\sqrt5-1)$ |
| Área de $z=x^2-y$, $\lvert y\rvert\le x\le1$ (§6) | $\sqrt6-\tfrac{\sqrt2}{3}$ |
| Longitud de la hélice $(2\cos t,2\operatorname{sen}t,2t)$, media vuelta | $2\sqrt2\,\pi$ |
| Área encerrada por $\lambda(t)=(t-t^2,\,t-t^4)$ | $1/30$ |
| Área encerrada por $\lambda(t)=(t-t^3,\,t-t^4)$ | $3/140$ |
| área$(D^*)$ sabiendo área$(D)$ | tabla en §1 (resultados $3$, $3/2$, $2$ y $4$) |
...
- Gauss con tapa: si el dato del flujo de la tapa viene orientado hacia $z^+$ pero el saliente del sólido ahí apunta hacia abajo → invertile el signo antes de restar.
...
- $x^2+y^2\le2x$ o $\le2y$ → **completá cuadrados antes de hacer nada**: son discos/cilindros corridos de radio 1 (vienen cayendo seguido).
...
*Generado el 2026-07-07, revisado el 2026-07-17. Fuentes: apuntes 01–19 (`apuntes/md/`), `examenes/INDICE.md`, `estrategia.md`, `que-saltear.md`. Teoría y demostraciones (T1/T2): checklist en `estrategia.md` y `repaso/flashcards.md`.*
